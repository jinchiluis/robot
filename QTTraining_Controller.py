...
class QTTraining_Controller(QMainWindow):
    ...
    def __init__(self, camera=None, calibration_data=None):
        ...
        # Data storage
        self.snapshots = {}  # {category: [(image_path, thumbnail_pixmap), ...]}
    ...
    def setup_ui(self):
        ...
        controls_layout.addWidget(self.new_category_btn)
        
        self.add_snapshot_btn = QPushButton("Add Snapshot")
        self.add_snapshot_btn.clicked.connect(self.add_snapshot_to_category)
    ...
    def take_snapshot(self):
        ...
        # Add to selected category or store temporarily
        selected = self.tree.currentItem()
        if selected and selected.parent() is None:  # Category selected
            category = selected.text(0).split(' (')[0]  # Remove count from category name
            self.add_snapshot(category, str(filepath), thumbnail)
        else:
            self.update_status(f"Snapshot saved: {filename} (select category to add)")
            # Store temporarily
    ...
    def add_snapshot_to_category(self):
        """Add the last snapshot to selected category."""
        selected = self.tree.currentItem()
        ...
        self.add_snapshot(category, filepath, thumbnail)
        
    def add_snapshot(self, category, filepath, thumbnail):
        """Add a snapshot to a category."""
        if category not in self.snapshots:
            self.snapshots[category] = []
        self.snapshots[category].append((filepath, thumbnail))
        
        # Update tree - just add the new image, don't rebuild everything
        if category in self.category_items:
            # Category already exists in tree, just add the image
            category_item = self.category_items[category]
            
            # Add new image as child
            image_count = len(self.snapshots[category])
            image_item = QTreeWidgetItem([f"Image {image_count}"])
            image_item.setIcon(0, QIcon(thumbnail))
            category_item.addChild(image_item)
            
            # Update category label with new count
            category_item.setText(0, f"{category} ({image_count})")
        else:
            # Category doesn't exist in tree yet, do full update
            self.update_tree()
            
        self.update_status(f"Added snapshot to {category}")
    ...
    def new_category(self):
        ...
        if ok and name:
            ...
            # Add to tree
            category_item = QTreeWidgetItem([f"{name} (0)"])
            self.tree.addTopLevelItem(category_item)
            self.category_items[name] = category_item
            category_item.setExpanded(True)
            
            # Select the new category
            self.tree.setCurrentItem(category_item)
            
            self.update_status(f"Created category: {name}")
    ...
    def rename_category(self):
        ...
        if ok and new_name and new_name != old_name:
            ...
            self.update_status(f"Renamed category: {old_name} → {new_name}")
    ...
    def delete_selected(self):
        ...
        if selected.parent() is None:
            # Category selected
            category = selected.text(0).split(' (')[0]  # Remove count
            reply = QMessageBox.question(self, "Confirm Delete", 
                                       f"Delete category '{category}' and all its images?")
            if reply == QMessageBox.Yes:
                # Delete image files
                for filepath, _ in self.snapshots[category]:
                    try:
                        os.remove(filepath)
                    except:
                        pass
                ...
                del self.snapshots[category]
                del self.category_items[category]
            ...
            # Image selected
            category_item = selected.parent()
            category = category_item.text(0).split(' (')[0]  # Remove count
            index = category_item.indexOfChild(selected)
            filepath, _ = self.snapshots[category][index]
            ...
            # Remove from data
            del self.snapshots[category][index]
            
            # Remove from tree
            category_item.removeChild(selected)
            
            # Update category label count
            count = len(self.snapshots[category])
            category_item.setText(0, f"{category} ({count})")
            
            # Re-number remaining images
            for i in range(category_item.childCount()):
                category_item.child(i).setText(0, f"Image {i+1}")
            
            self.update_status(f"Deleted image from {category}")
            
    def clear_all(self):
        """Clear all data after confirmation."""
        reply = QMessageBox.question(self, "Confirm Clear All", 
                                   "Delete all categories and images?")
        if reply == QMessageBox.Yes:
            # Delete all files
            for category in self.snapshots:
                for filepath, _ in self.snapshots[category]:
                    try:
                        os.remove(filepath)
                    except:
    ...
    def update_tree(self):
        ...
        for category, images in self.snapshots.items():
            if category == "_temp_snapshots":
                continue
                
            # Create category item
            category_item = QTreeWidgetItem([f"{category} ({len(images)})"])
            self.tree.addTopLevelItem(category_item)
    ...
    def train_model(self):
        ...
        valid_categories = {k: v for k, v in self.snapshots.items() 
                          if k != "_temp_snapshots" and v}
        
        if len(valid_categories) < 2:
            QMessageBox.warning(self, "Warning", 
                              "Need at least 2 categories with images to train")
            return
            
        # Prepare training data
        training_data = {}
        for category, images in valid_categories.items():
            training_data[category] = [filepath for filepath, _ in images]
