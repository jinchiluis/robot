...
class ClickableLabel(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if self.hover_pos:
            ...
            rect.adjust(-2, -2, 2, 2)
            rect.moveTopLeft(self.hover_pos)
            painter.drawRect(rect)
            painter.drawText(self.hover_pos[0], self.hover_pos[1], text)
...
class QTCalibration_Controller(QWidget):
    ...
    def __init__(self, camera_widget, parent=None):
        """
        Initialize calibration widget.
        
        Args:
            camera_widget: QTCamera instance to get frames from
